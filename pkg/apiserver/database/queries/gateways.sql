...
UPDATE gateways
SET public_key = @public_key, endpoint = @endpoint, ip = @ip, requires_privileged_access = @requires_privileged_access, password_hash = @password_hash
WHERE name = @name;
...
VALUES (@name, @endpoint, @public_key, @ip, @password_hash, @requires_privileged_access)
ON CONFLICT (name) DO

...
UPDATE gateways
SET public_key = @public_key, endpoint = @endpoint, ip = @ip, ipv6 = @ipv6, requires_privileged_access = @requires_privileged_access, password_hash = @password_hash
WHERE name = @name;
...
INSERT INTO gateways (name, endpoint, public_key, ip, ipv6, password_hash, requires_privileged_access)
VALUES (@name, @endpoint, @public_key, @ip, @ipv6, @password_hash, @requires_privileged_access)
ON CONFLICT (name) DO

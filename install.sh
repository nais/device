...
err=$(/usr/bin/osascript -e "do shell script \"pkill device-agent; pkill device-agent-helper; installer -target / -pkg '$temp_pkg'\" with prompt \"naisdevice wonders if anyone ever reads this message? also it would like to install itself 😎\" with administrator privileges") && ok || fail

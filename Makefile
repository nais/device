...
local-apiserver:
	...
	go run ./cmd/apiserver/main.go --db-connection-uri=postgresql://postgres:postgres@localhost/postgres --bind-address=127.0.0.1:8080 --slack-token=${APISERVER_SLACK_TOKEN} --config-dir=${confdir} --development-mode=true
	echo ${confdir}

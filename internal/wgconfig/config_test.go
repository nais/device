	...
	cfg := &wireguard.Config{
		...
		Peers: []wireguard.Peer{
			&pb.Gateway{
				...
				Ipv4:       "10.255.24.30",
				RoutesIPv4: []string{"192.168.0.0/24"},
			},
		},
	}

	wgCfg, err := wgconfig.BuildServerConfig(cfg)
	require.NoError(t, err)

	assert.NotNil(t, wgCfg.ListenPort)
	assert.Equal(t, 51820, *wgCfg.ListenPort)
	assert.True(t, wgCfg.ReplacePeers)
	require.Len(t, wgCfg.Peers, 1)
}

func TestBuildServerConfig_NoListenPort(t *testing.T) {
	cfg := &wireguard.Config{
		PrivateKey: testPrivateKey,
		Peers:      []wireguard.Peer{},
	}
